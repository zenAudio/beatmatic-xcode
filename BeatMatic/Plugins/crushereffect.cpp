...
void CrusherEffect::getNextAudioBlock(const AudioSourceChannelInfo& bufferToFill) {
    ...
    std::cout << "MPD: NATIVE: CPP: CrusherEffect::getNextAudioBlock: entering loop: " << i0 << "; N=" << N << ", " << M << ", parms.rate=" << parms.rate << std::endl;
    for (int i = 0; i < N; i++) {
        float v = 0;
        ...
        if (cnt >= 1) {
            ...
            std::cout << "MPD: NATIVE: CPP: CrusherEffect::getNextAudioBlock: " <<  "y=" << y << "; v=" << v << "; m=" << m << std::endl;
        }
        for (int chan = 0; chan < M; chan++) {
            buf[chan][i0 + i] = y;
        }
